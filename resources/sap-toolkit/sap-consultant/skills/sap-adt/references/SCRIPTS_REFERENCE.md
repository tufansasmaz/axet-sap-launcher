# SAP ADT Scripts Reference — ARCHIVED CLI catalog

> **⚠️ ARCHIVED (v1.3.0).** The per-operation CLI scripts below were retired when the
> plugin became MCP-only — every operation now goes through the `adt_*` MCP tools
> (see [../SKILL.md](../SKILL.md)). The scripts live in **`scripts/_archived_cli/`**,
> NOT on the active scripts path — `cd scripts/ && python download_object.py` fails
> by design.
>
> **When this page is still useful:**
> 1. An old doc names one of these scripts and you need the MCP replacement — same
>    stem: `download_object.py` → `adt_get_source`, `run_sql_query.py` → `adt_sql`,
>    `push_object.py` → `adt_push`, `list_transports.py` → `adt_list_transports`, …
> 2. The MCP server is genuinely down and you need a one-shot fallback. Then set
>    `SAP_ADT_SCRIPTS_DIR` to the **archived** dir of the **newest** installed
>    version (`sort -V | tail -1`, never `head -1`):
>
>    ```bash
>    SAP_ADT_SCRIPTS_DIR="$(ls -d ~/.claude/plugins/cache/*/sap-consultant/* | sort -V | tail -1)/skills/sap-adt/scripts/_archived_cli"
>    ```
>
> Every example below assumes that definition. **Each CLI call is a fresh process** —
> no persistent session — so per-process pushes produce the false `NO_LOCK_SUPPORT`/403
> artifacts described in SKILL.md ("When a connection/write FAILS"). Never draw
> conclusions about SAP policy from CLI behavior, and never push through the CLI when
> the MCP is available.

**All scripts support:** `--cwd` (working dir containing `.conn_adt`), ASCII-only
output (`[OK]`, `[FAIL]`, `[ERROR]`), exit codes 0/1, `--help`.

---

## Core Libraries (live engine, on the ACTIVE scripts path)
...
**Note:** `create_service_definition.py` and `create_service_binding.py` were never
implemented as CLI scripts (only `create_behavior_definition.py` is in the archive) —
use the MCP `adt_create` for all RAP types. See [RAP_CDS_OBJECTS.md](RAP_CDS_OBJECTS.md)
for confirmed API details.
...
## Python Usage Pattern (engine-level, advanced)

Driving `SAPClient` directly is for working ON the engine, not everyday SAP work —
the MCP tools cover that. Note the import root here is the **ACTIVE** scripts dir
(`skills/sap-adt/scripts/`, where `sap_client.py` lives), NOT `_archived_cli/`.

**CRITICAL ORDER:** Set the working directory BEFORE any SAP imports:

```python
import sys
sys.path.insert(0, r'<...>/skills/sap-adt/scripts')

from sap_adt_lib import set_explicit_working_dir
set_explicit_working_dir(r'<PROJECT_DIR>')
...
```bash
cd "<...>/skills/sap-adt/scripts" && python - <<'PY'
import sys
sys.path.insert(0, '.')
from sap_adt_lib import set_explicit_working_dir
set_explicit_working_dir(r'<PROJECT_DIR>')
