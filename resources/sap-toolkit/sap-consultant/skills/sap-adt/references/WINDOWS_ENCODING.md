...
**This page is about ONE thing: what Python `print()`s to a Windows console.**
Windows consoles use `cp1252`/`cp857`, which cannot render Unicode symbols, so a
stray checkmark crashes the script with `UnicodeEncodeError`.

> ## It does NOT apply to data you write into SAP
>
> Object **descriptions**, text elements, titlebars, message texts and ABAP source
> comments are written in the **login language, with that language's own characters**
> — `İ ı ş ğ ü ö ç`. SAP stores them correctly; the customer's own developments in
> the same system look like *"Mail Gönderme İşlemleri"*.
>
> Stripping them produces *"Ihtiyac Karsilama Takip Raporu"* in a Turkish system,
> which is a defect. Object **names** stay ASCII — that is SAP's own constraint, not
> this rule. Full rule: naming standard §3.3.
>
> The line is simple: **console = ASCII · payload = the user's language.**
...
**BEST PRACTICE:** Use ASCII in `print()` — it always works.
(Again: this is about console output only. See the box at the top.)
