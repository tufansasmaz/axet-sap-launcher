...
Package names follow the corporate naming standard (§2 Paket Adlandırma in
`NAMING_STANDARD.md`, shipped with the sap-specs `ts-generator` skill):
`Z<Module><PkgNo>` cloud packages under module packages — e.g. `ZSD001`, no
underscore before the number. The project brief supplies the actual package.

The `ZAI` / `ZAI_T` / `ZAI_TB` scheme seen in older examples is a legacy sandbox
layout — do not copy it to customer projects.
