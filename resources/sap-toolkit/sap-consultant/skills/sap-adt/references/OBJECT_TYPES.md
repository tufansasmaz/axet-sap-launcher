...
A function module has **no URI of its own** — it is addressed only under its group:

- Object: `/sap/bc/adt/functions/groups/{function_group}/fmodules/{name}`
- Source: `/sap/bc/adt/functions/groups/{function_group}/fmodules/{name}/source/main`

> Until 2026-08-19 this section documented `/sap/bc/adt/functions/modules/{name}` as
> fact. That path exists on no system; it returns 404, which reads as "the module is
> not there" and sends the reader looking for the object instead of the URL. The
> group-less form can no longer be built — `get_object_url()` raises and names the
> `function_group` argument.
>
> **Reading** an FM: `adt_get_source(name=…, object_type="function", function_group=…)`.
> **Writing** one: `adt_write_function_module` — `adt_push` cannot, and will say so.
> **Finding the group:** `adt_sql("SELECT pname FROM tfdir WHERE funcname = 'ZFM'")`,
> then drop the leading `SAPL` from `PNAME`.
>
> Do not reach for the group's generated include (`L<group>U01`, `L<group>U02`, …).
> Those are SAP's compile-time artefacts, and the engine's own Z/Y namespace guard
> rejects them before the request leaves the machine — the refusal looks like SAP's
> and is not.
