...
def get_package_info(adt: SAPADTClient, package_name: str) -> dict:
    ...
    response = adt.session.get(url, headers=headers, timeout=30)
...
def validate_package(adt: SAPADTClient, name: str, description: str,
    ...
    response = adt._request_with_csrf_retry(
        'post', url, headers=headers, params=params, timeout=30)
...
def create_package(adt: SAPADTClient, name: str, description: str,
    ...
    # pak:recordChanges — "record object changes in transport requests". SAP does
    # not derive it, and some systems reject the POST without it while others
    # accept it (this script created packages fine without it several times before
    # a system refused). Hardcoded true rather than derived: both entrypoints
    # REQUIRE a transport (--transport, and require_transport in the MCP guard), so
    # a package created here is transportable by construction and false would be
...
{super_pkg_ref}
<pak:attributes pak:packageType="{package_type}" pak:recordChanges="true"/>
{super_pkg_elem}
...
<pak:subPackages/>
    ...
    response = adt._request_with_csrf_retry(
        ...
        data=xml_body.encode('utf-8'), timeout=60)
