...
Usage:
    python run_sql_query.py --query "SELECT * FROM ZAI_T_CONFIG" --max-rows 10 --cwd /path/to/project
    python run_sql_query.py --query "SELECT * FROM ZAI_T_HISTORY" --max-rows 5 --format json --cwd /path/to/project
    python run_sql_query.py --query "SELECT * FROM ZAI_T_HISTORY" --max-rows 5 --columns UNAME,MODEL --cwd /path/to/project
"""
import argparse
import csv
import json
...
def format_json(result, selected_columns=None):
    ...
    rows = []
    for row in data:
        # Guard against ragged rows (SAP may return fewer cells than columns
        # when trailing columns are NULL) — mirrors the table formatter's guard.
        rows.append({columns[i]: (row[i] if i < len(row) else None) for i in range(len(columns))})
...
def format_csv(result, selected_columns=None):
    ...
    for row in data:
        # Pad ragged rows to the column count so columns stay aligned.
        writer.writerow([(row[i] if i < len(row) else None) for i in range(len(columns))])
