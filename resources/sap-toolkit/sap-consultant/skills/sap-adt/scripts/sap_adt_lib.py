...
def _adt_reason(response):
    """SAP's own reason for refusing an ADT request, in one line.

    ADT wraps the useful sentence in <message> (or <faultstring> for classic ABAP
    exceptions) and buries it under 300 characters of namespace declarations. Two
    very different refusals arrive with the same HTTP status, so the sentence is the
    only thing that tells them apart:
      * "CSRF token validation failed"        -> stale token; refetch and retry
      * "user X is already editing <object>"  -> a real edit lock; SE37/SM12
    Returns a short, already-trimmed string; never raises.
    """
    try:
        body = getattr(response, 'text', '') or ''
        return (adt_reason_from_text(body)
                or f'HTTP {getattr(response, "status_code", "?")}')
    except Exception:
        return f'HTTP {getattr(response, "status_code", "?")}'


def adt_reason_from_text(body: str) -> str:
    """The same extraction, on a body we already hold rather than a response.

    Split out so SAPADTError can run it over the response_text it was handed.
    Until 2026-09-03 that text was captured on every raise and shown by nothing:
    __str__ returned "[400] Failed to run query" whether the SQL had a typo, the
    table did not exist, or the user lacked the authorisation -- and the sentence
    that told them apart was sitting in the exception the whole time.

    Two things fight for the same 500 characters, which is why the plain-text
    fallback is trimmed to the END rather than the start: SAP's XML envelope is
    ~200 characters of namespace declarations, and its HTML error page opens with
    a stylesheet, so the first few hundred characters of either are furniture.
    """
    import re as _r
    from html import unescape as _u
    if not body:
        return ''
    m = (_r.search(r'<(?:\w+:)?message[^>]*>(.*?)</(?:\w+:)?message>', body, _r.S)
         or _r.search(r'<(?:\w+:)?faultstring[^>]*>(.*?)</(?:\w+:)?faultstring>', body, _r.S)
         or _r.search(r'<(?:\w+:)?localizedMessage[^>]*>(.*?)</(?:\w+:)?localizedMessage>', body, _r.S))
    if m:
        return ' '.join(_u(m.group(1)).split())[:300]
    # No envelope: strip tags, drop CSS/script, and keep the sentence.
    text = _r.sub(r'<(script|style)[^>]*>.*?</\1>', ' ', body, flags=_r.S | _r.I)
    # A tag left unterminated by the capture cut. SAP's HTML error page embeds
    # its logo as a base64 data: URI, and the 4000-character capture lands in the
    # middle of it -- so `<[^>]+>` finds no closing bracket, leaves the fragment
    # alone, and 300 characters of base64 become the error message. Drop the
    # dangling fragment before stripping the well-formed tags around it.
    text = _r.sub(r'<[^>]*$', ' ', text)
    text = _r.sub(r'<[^>]+>', ' ', text)
    # Belt: any data: URI that survived (an attribute the tag stripper missed).
    text = _r.sub(r'data:[^\s"\']{40,}', ' ', text)
...
class SAPADTError(Exception):
    ...
    def __str__(self):
        """Our label AND what SAP actually said, when the two differ.

        The label alone is a category ("Failed to run query", "Failed to create
        CLAS/OC"), and several unrelated refusals share one. SAP's own sentence
        is the part that tells the consultant what to change, and it was being
        captured into response_text on every raise and printed nowhere -- which
        is what a bare "provider error" looks like from the inside. Measured on
        NS4 2026-09-03: a mistyped WHERE clause and a non-existent table both
        reported "[400] Failed to run query" while the body held
        '"WHEREE" is invalid here (due to grammar). ADT_DATAPREVIEW_MSG 004'.

        Suppressed when SAP adds nothing: an empty body, or a sentence already
        contained in the label (the CSRF path raises with SAP's own words).
        """
        head = f"[{self.status_code}] {self.message}" if self.status_code else self.message
        try:
            reason = adt_reason_from_text(self.response_text or '')
        except Exception:
            reason = ''
        if reason and reason.lower() not in (self.message or '').lower():
            return f"{head} — SAP: {reason}"
        return head
...
def set_explicit_working_dir(path: str) -> None:
    ...
    if conn_path.exists():
        # override=True is load-bearing. os.environ already holds whatever the
        # FIRST .conn_adt put there -- module import, or an earlier switch -- and
        # python-dotenv's default (override=False) leaves those values in place.
        # Without it this function re-points conn_path to the new file and then
        # reads the OLD credentials back out of os.environ: the caller asks for
        # system A and silently gets system B. Measured 2026-08-11 pointing at a
        # source system and reaching the target instead, which in a cross-system
        # transfer is the wrong-system write this whole engine guards against.
...
class SAPADTClient:
    ...
    def __init__(self, url=None, user=None, password=None, client=None, language=None,
        ...
        self._csrf_healing = False
        self._install_csrf_recovery()
    ...
    def _get_headers(self, accept_type="application/vnd.sap.adt.core.v1+xml", content_type=None):
        ...
        sap-language mirrors sap-client: ICF honors it as a logon-language override
        so newly created repository objects get adtcore:masterLanguage = this value
        instead of falling back to the connecting user's SU01 default logon language
        (USR01-LANGU). Without it, .conn_adt's ADT_SAP_LANGUAGE was written into the
        create-object XML body but silently ignored by SAP — the session (and thus
        every object created in it) still inherited the user's SU01 language.
        """
        ...
        if not is_saml:
            ...
            if self.language:
                headers['sap-language'] = self.language
    ...
    def _install_csrf_recovery(self) -> None:
        """Retry ANY mutating request once on 403 'CSRF token validation failed'.

        Sixteen of the engine's thirty-three mutating calls carried their own
        inline retry and seventeen did not, so whether a stale token surfaced
        as "forbidden" depended on which tool the consultant happened to reach
        for. `unlock_object` was one of the seventeen, and that one is why a
        restart did not always help: its 403 left the object LOCKED on SAP, and
        an ENQUEUE lock outlives the process that took it.

        Wrapping the session covers all thirty-three and every call added later.
        A call site cannot forget what it does not have to remember -- which is
        the whole reason this lives here rather than in seventeen more places.

        GETs are untouched (they need no token), and the refresh itself is a GET
        so it cannot recurse. `_csrf_healing` guards against re-entry anyway.
        """
        inner = self.session.request

        def request(method, url, **kw):
            response = inner(method, url, **kw)
            if (response.status_code != 403
                    or str(method).upper() not in ('POST', 'PUT', 'DELETE')
                    or self._csrf_healing
                    or 'CSRF' not in (response.text or '')):
                return response
            self._csrf_healing = True
            try:
                self.fetch_csrf_token(force_refresh=True)
            except Exception:
                # Cannot refresh -- hand back the original 403 rather than a
                # connection error, so the caller reports what SAP actually said.
                return response
            finally:
                self._csrf_healing = False
            ...
            headers['X-CSRF-Token'] = self.csrf_token
            if self.debug_enabled:
                self._debug(f"[DEBUG] CSRF 403 on {str(method).upper()} {url} — "
                            f"refreshed and retried once")
            return inner(method, url, headers=headers, **kw)

        self.session.request = request

    def _session_is_restored(self) -> bool:
        """True when this process starts with the SAME SAP session it had before.

        A CSRF token is bound to the SAP session that issued it. Caching the
        token across processes is therefore only sound when the SESSION comes
        back too. SAML does that: the cookies are persisted next to the project
        and reapplied in __init__ (`_apply_saml_cookies`). Basic auth does not
        -- every process opens a brand new session, so a token read back off
        disk belongs to a session SAP has already forgotten, and every write
        answers 403 "CSRF token validation failed".

        That is the shape consultants reported as "the kit says forbidden":
        restarting aXet made a THIRD session and loaded the same dead token, so
        the restart appeared not to help, and it kept not helping until the 6h
        TTL ran out. Measured on NS4 2026-09-03: token from session A + a fresh
        cookie jar -> 403; the same request after force_refresh -> 200.
        """
        return bool(self._auth_provider
    ...
    def _load_csrf_from_cache(self) -> Optional[str]:
        ...
        if not self._session_is_restored():
            return None
    ...
    def fetch_csrf_token(self, force_refresh: bool = False):
        ...
        # /discovery serves ONE representation and rejects anything else. The
        # default Accept (adt.core.v1+xml) makes it answer 406
        # ExceptionResourceNotAcceptable "Accepted content types:
        # application/atomsvc+xml". Today that is survivable by accident: this
        # ICF stack still emits X-CSRF-Token on the 406, and the code below reads
        # the header without checking the status. Anything that rewrites a
        # non-2xx response -- a reverse proxy, a WAF, an F5 error page -- drops
        # the header instead, fetch_csrf_token returns None, and every write
        # fails 403 with no way back. Measured on NS4 2026-09-03:
        # core.v1+xml -> 406, atomsvc+xml -> 200, both carrying the token.
        # compatibility/graph (the SAML/BTP endpoint) is happy with the default.
        headers = self._get_headers(
            'application/atomsvc+xml' if csrf_endpoint.endswith('/discovery')
            else "application/vnd.sap.adt.core.v1+xml")
    ...
    def list_class_includes(self, class_name):
        """The includes SAP advertises for a class, with the URI SAP gives for each.

        A class is not one source file. `source/main` is the global class only;
        local class definitions and implementations, macros and test classes each
        live behind their own endpoint. For a RAP behaviour pool that is the whole
        story -- main is a generated shell and every line of the handler is in the
        `implementations` include.

        The list is READ, never assumed. Measured live on S/4HANA 2023: a class
        without test classes does not advertise `testclasses`, and fetching it
        anyway returns 404. Equally, the URI is taken from SAP's own
        `abapsource:sourceUri` rather than built: the include endpoint is
        `.../includes/implementations` with NO `/source/main` suffix, which is not
        what the pattern for the class itself would lead you to write.

        Returns a list of {includeType, sourceUri, changedAt, changedBy}, `main`
        included, in the order SAP lists them. Empty list if the class resource
        cannot be read -- callers treat that as "no include information", never as
        "no includes".
        """
        url = f"{self.url}/sap/bc/adt/oo/classes/{class_name.lower()}"
        try:
            response = self.session.get(
                url, headers=self._get_headers('application/*'),
                timeout=self.timeout_short)
        ...
        out = []
        for element in re.findall(r'<[A-Za-z]*:?include\b[^>]*>', response.text):
            itype = attr(element, 'includeType')
            uri = attr(element, 'sourceUri')
            if not itype or not uri:
                continue
            out.append({
                'includeType': itype,
                'sourceUri': uri,
                'changedAt': attr(element, 'adtcore:changedAt'),
                'changedBy': attr(element, 'adtcore:changedBy'),
            })
        return out

    def get_class_include_source(self, class_name, source_uri):
        """Fetch one class include, addressed by the sourceUri SAP advertised.

        Never build this URI. See list_class_includes() for why.
        """
        url = (f"{self.url}/sap/bc/adt/oo/classes/{class_name.lower()}/"
               f"{source_uri.lstrip('/')}")
        response = self.session.get(
            url, headers=self._get_headers('text/plain'),
            timeout=self.timeout_short)
        if response.status_code != 200:
            raise SAPADTError(
                f"Failed to read include {source_uri} of {class_name}",
                status_code=response.status_code,
                endpoint=url,
    ...
    def _verify_and_return_lock(self, response, object_url, transport, label=''):
        ...
        if corrnr_actual and transport and corrnr_actual.upper() != transport.upper():
            ...
            if is_link_up == 'X':
                ...
                # IS_LINK_UP='' does NOT mean "a different transport of the same
                # user". Per is_obj_recorded_in_foreign_req it means the caller
                # HOLDS A TASK in the request that holds the object -- and on a
                # shared request that request belongs to the lead, not to the
                # caller. This branch used to say "your own transport", which on
                # NS4 named D_ONERO's NS4K900214 to the person who did not own it
                # (2026-08-21, addendum 2 of the transport-pin proposal). The two
                # readings differ in exactly the shared-request shape, which is
                # the shape that actually bites.
                raise SAPLockError(
                    f"SAP assigned transport {corrnr_actual} but {transport} was requested.\n\n"
                    f"You HOLD A TASK in {corrnr_actual} (IS_LINK_UP is empty), so SAP\n"
                    f"attributes the object there and overrides the corrNr hint. That is\n"
                    f"NOT the same as {corrnr_actual} being yours: on a shared request the\n"
                    f"K-request belongs to the lead and you own only your task under it.\n\n"
                    f"Fix:\n"
                    f"  1. Find out whose it is before touching it:\n"
                    f"     SELECT as4user FROM e070 WHERE trkorr = '{corrnr_actual}'\n"
                    f"  2. Yours: SE01/SE09 → open {corrnr_actual} → move/delete the object\n"
                    f"     entry, SM12 → release the enqueue lock, retry with {transport}\n"
                    f"  3. A colleague's: use {corrnr_actual} deliberately, or coordinate\n"
                    f"     with the owner. Do NOT move an object out of someone else's\n"
                    f"     request.\n\n"
    ...
    # SAP transport request: 3-char system id (letter first, digits allowed after,
    # e.g. DS4, NS4) + 'K' + the number. The previous pattern demanded [A-Z]{3}
    # followed by digits, which cannot match DS4K900067 at all -- it only ever
    # matched through its own [A-Z0-9]{8,} fallback, which also matches any long
    # token that happens to follow the word "transport".
    _TRKORR_RE = re.compile(r'\b([A-Z][A-Z0-9]{2}K\d{5,})\b')

    def _transport_conflict_message(self, object_url, requested, response_text):
        """Explain a 409 by asking CTS who holds the object, not by reading prose.

        The old message guessed the holder with a regex over SAP's response and then
        recommended the most invasive repair first: clear the enqueue lock in SM12,
        then move the object between requests in SE09. On a customer system in
        2026-08 that advice cost an afternoon. What actually worked was the option
        it never mentioned -- the object was already recorded in the caller's OWN
        request, a different one from the session pin, and passing that request
        made the write succeed with nothing to repair.

        So: ask `transportchecks` (authoritative, read-only, the same source
        adt_transport_check uses), lead with the cheap fix, and separate the two
        kinds of lock -- a CTS request binding is released in SE03, never SM12.
        """
        holder = owner = None
        foreign = False
        try:
            chk = self.transport_check(object_url)
            for lock in ((chk or {}).get('locks') or []):
                if lock.get('trkorr'):
                    holder = lock['trkorr']
                    owner = lock.get('as4user') or ''
                    foreign = bool(lock.get('foreign'))
                    break
        except Exception:
            pass                      # falls through to the response-text guess

        source = 'CTS'
        if not holder:
            m = self._TRKORR_RE.search(response_text or '')
            holder, source = (m.group(1) if m else None), 'response text'

        if self.debug_enabled:
            self._debug(f"[DEBUG] lock_object 409 - holder={holder} via {source}, "
                        f"requested={requested}, foreign={foreign}")

        head = (f"Object is already recorded in transport {holder}"
                f"{' (owner ' + owner + ')' if owner else ''}"
                if holder else "Object is locked under a transport this call could not identify")
        lines = [f"{head}; this call sent {requested or 'no transport'}.", ""]

        if holder and not foreign:
            lines += [
                f"That request is yours, so the cheap fix is to use it:",
                f"  pass transport='{holder}' on this call, or adt_set_transport('{holder}').",
                f"Nothing needs repairing -- an object may only be recorded in one",
                f"modifiable request at a time, and it is already in one of yours.",
                ""]
        elif holder and foreign:
            lines += [
                f"That request belongs to {owner or 'another user'}. Do not take it over.",
                f"Either have them release it, or ask them to add your change to it.",
                ""]

        lines += [
            "If the object genuinely has to move requests, that is the expensive path:",
            f"  SE03 -> Transport Organizer Tools -> Unlock Objects, for the CTS binding",
            f"  (SM12 clears an ENQUEUE lock, which is a different thing and will not",
            f"  release a request binding).",
            "",
            "Diagnose before retrying: adt_transport_check(name=..., object_type=...)",
            "reports the holder directly. A blind retry can spawn a ghost transport."]
        return "\n".join(lines)
    ...
    def lock_object(self, object_url, access_mode='MODIFY', transport=None, max_attempts=4, allow_no_transport=False):
        ...
        # Forget the PREVIOUS lock before attempting this one. These three are
        # written only by _verify_and_return_lock, which runs only on a 200 -- so
        # after a 409, a 403 or a lock that fails outright they still describe the
        # object locked before this one, and every caller that reads them is then
        # deciding about object B from object A's answer.
        #
        # push_object is the one that bites: its Bug 11 branch catches the 409,
        # reads a stale _last_lock_effective_transport, finds it differs from the
        # transport just requested, and RETRIES THE LOCK WITH AN UNRELATED
        # TRANSPORT -- swallowing the 409 message on the way. That message is the
        # one rewritten after it cost a consultant an afternoon on DS4, and this
        # is why nobody had seen it since. Cleared here rather than in each
        # caller, because the invariant belongs to the lock, not to the readers.
        self._last_lock_corrnr = None
        self._last_lock_is_link_up = None
        self._last_lock_effective_transport = None
        ...
        for attempt_num, (endpoint, use_query_param, description) in enumerate(lock_strategies, 1):
            ...
            try:
                ...
                # 409 - Transport conflict: object locked under a different transport
                    ...
                    response_text = response.text if response.text else ''
                    raise SAPLockError(
                        self._transport_conflict_message(object_url, transport, response_text),
                        status_code=409, response_text=response_text[:500])
...
<adtcore:objectReferences xmlns:adtcore="http://www.sap.com/adt/core">
  <adtcore:objectReference adtcore:uri="{object_url}" adtcore:type="{object_type}" adtcore:name="{object_name}" adtcore:parentUri="{parent_uri}"/>
    ...
    def activate_object(self, object_name, object_url):
        ...
        """
        # `object_url` may be a LIST of (name, url) pairs, in which case every one of
        # them seeds the same pre-audit. That is what lets SAP resolve references
        # BETWEEN the objects: activating a domain, its data element and the table
        # over both, one at a time, fails on the table until the other two are active,
        # and the failure ("Nametab cannot be generated") names neither of them.
        # Sent together, SAP orders them itself.
        if isinstance(object_url, (list, tuple)):
            seeds = [(n, u) for n, u in object_url]
            if not seeds:
                return {'success': False, 'activation_executed': False,
                        'check_executed': False, 'generation_executed': False,
                        'errors': [{'message': 'no objects to activate', 'type': 'E',
                                    'object': '', 'line': '0', 'href': ''}],
                        'warnings': [], 'response': ''}
            # The first seed labels the errors and the 403 recovery path.
            object_name, object_url = seeds[0]
        else:
            seeds = [(object_name, object_url)]
        ...
        seed_refs = ''.join(
            f'<adtcore:objectReference adtcore:uri="{u}"'
            f' adtcore:type="{self._extract_object_type(u)}"'
            f' adtcore:name="{n}"'
            f' adtcore:parentUri="{self._extract_parent_uri(u)}"/>'
            for n, u in seeds
        )
        seed_body = (
        ...
        if r1.status_code != 200:
            return {
                ...
                'errors': [{'message': f'HTTP {r1.status_code} - Activation pre-audit failed',
                            'type': 'E', 'object': object_name, 'line': '0', 'href': ''}],
                'warnings': [], 'response': r1.text[:4000]
            }
        ...
        if p1['ioc_refs']:
            ...
            # Every seed, not just the first -- otherwise a multi-object activation
            # whose pre-audit returned no sub-objects would quietly activate one.
            refs = [(u, self._extract_object_type(u), n) for n, u in seeds]
        ...
        if r2.status_code != 200:
            return {
                ...
                'errors': [{'message': f'HTTP {r2.status_code} - Batch activation failed',
                            'type': 'E', 'object': object_name, 'line': '0', 'href': ''}],
                'warnings': [], 'response': r2.text[:4000]
            }
    ...
    def _handle_activation_403(self, response, object_name, object_url, activation_body, headers):
        ...
        return {
            ...
            'warnings': [], 'response': response.text[:4000]
        }

    def search_objects(self, query, max_results=100, obj_type=None):
        """Search for ABAP objects, optionally narrowed to one ADT type.

        obj_type goes to SAP as `objectType` rather than being filtered out of the
        answer here. quickSearch caps at maxResults BEFORE any client-side filter
        runs, so on a system with thousands of Z objects the cap was spent on
        whatever sorted first and the filter then found nothing: measured on NS4
        2026-09-03, `Z*` + PROG/P returned 0 rows at maxResults=10, 1 at 200 and
        12 at 1000, while the same search with objectType=PROG/P returned 50 rows
        of PROG/P at maxResults=50. Nothing in the empty answer said it had been
        truncated, so it read as "this system has no Z programs".

        Accepts either granularity SAP does -- 'PROG' matches PROG/I and PROG/P,
        'PROG/P' just the one.
        """
        ...
        if obj_type:
            params['objectType'] = str(obj_type).strip().upper()
    ...
    def remove_object_from_transport(self, object_name, transport, object_type='class'):
        ...
        otype = type_map.get((object_type or '').lower())
        if not otype:
            # Accept a RAW SAP type too (DEVC, TABL, DOMA, DDLS ...). Stripping
            # entries out of a request means working from E071, where the type is
            # already in SAP's own spelling -- mapping it back to an engine name
            # first would need a table covering every type SAP has.
            raw = (object_type or '').strip().upper()
            if raw.isalnum() and 3 <= len(raw) <= 4:
                otype = raw
        if not otype:
    ...
    def create_object(self, obj_type, name, package_name, description, package_path, responsible=None, transport=None, max_retries=10):
        ...
        if obj_type == 'PROG/P':
            ...
            # Point at the tool that DOES create this type instead of stopping dead.
            # This generic path only knows the four source objects below; every other
            # type has a dedicated creator, and finding that out used to cost a round
            # trip against SAP and a search through the tool list.
            dedicated = {
                'DOMA/DD': 'adt_create_domain',
                'DTEL/DE': 'adt_create_data_element',
                'TABL/DT': 'adt_create_ddic_shell (object_type="table")',
                'TABL/DS': 'adt_create_ddic_shell (object_type="structure")',
                'TTYP/DA': 'adt_create_table_type',
                'DDLS/DF': 'adt_create_cds_view',
                'DCLS/DL': 'adt_create_access_control',
                'DDLX/EX': 'adt_create_metadata_extension',
                'BDEF/BDO': 'adt_create_behavior_definition',
                'ENQU/DL': 'adt_create_lock_object',
                'TYPE/DT': 'adt_create_type_group',
                'MSAG': 'adt_message_class',
                'FUGR/F': 'adt_create_function_group',
                'DEVC/K': 'adt_create_package',
                'SRVB/SVB': 'adt_create_service_binding',
            }
            tool = dedicated.get(obj_type)
            if tool:
                raise ValueError(
                    f"Unsupported object type here: {obj_type}. This generic path handles "
                    f"PROG/P, PROG/I, CLAS/OC and INTF/OI only - use {tool} for this type."
                )
            raise ValueError(
                f"Unsupported object type: {obj_type}. This generic path handles PROG/P, "
                f"PROG/I, CLAS/OC and INTF/OI; other types have dedicated creators "
                f"(adt_create_domain, adt_create_cds_view, adt_create_table_type, ...). "
                f"If none fits, the type has no creator on this engine yet."
            )
  ...
  <adtcore:packageRef adtcore:name="{package_upper}" adtcore:uri="{package_path}"/>
    ...
    def create_transport(self, description, package_name=None, transport_layer=None,
        ...
        if self._is_btp_cloud_url():
            return self._create_transport_modern(description, request_type, task_type, target)
        ...
        # _create_transport_modern sends.
        #
    ...
    def _create_transport_modern(self, description, request_type='K', task_type='S', target=''):
        """Transport creation via the MODERN tm:root endpoint /cts/transportrequests.

        Not BTP-specific despite the old name: this is the path the live ADT
        handler takes on-prem too (CL_CTS_ADT_TM_RES_COLL_CONT->post). The
        legacy asx:abap variant is _create_transport_onprem, tried only when
        this one is genuinely absent (404/405/501).
        """
        ...
        if response.status_code in (200, 201):
            ...
            # 406 = SUCCESS with the transport number in the Location header. NOT a
            # BTP-only behaviour: NS4, an on-prem 816 stack, answers 406 here too, and
            # labelling it '(BTP Cloud)' told consultants their on-prem system was
            # something it is not. The status describes the ENDPOINT's reply, not the
            # kind of system.
            trkorr = self._extract_transport_from_location(location)
            if trkorr:
                return {'success': True, 'transport': trkorr,
                        'message': f'Transport {trkorr} created successfully'}
            raise SAPTransportError(
                'Transport creation returned HTTP 406 (which this endpoint uses for '
                'success) but no transport number was in the Location header: '
                f'"{location}"',
                status_code=406,
                response_text=response.text[:4000]
            )
    ...
    def delete_object(self, object_url, lock_handle, transport=None):
        ...
        def try_delete(lock_as_param: bool, lock_as_header: bool, corr_as_param: bool):
            ...
            if self.debug_enabled:
                self._debug(f"[DEBUG] delete_object response - status: {response.status_code}, text: {response.text[:4000]}")
        ...
        # `is not None`, NOT a truth test. requests.Response.__bool__ returns
        # self.ok, so a 400 is FALSY -- and this loop only gets here when every
        # attempt failed, which is exactly when the response is falsy. Both lines
        # therefore threw away a perfectly good response and reported 'unknown' /
        # 'no response'. SAP's own sentence was present five times over and
        # printed none of them. See PROPOSAL.md fault 1.
        status = last_response.status_code if last_response is not None else 'unknown'
        text = last_response.text if last_response is not None else 'no response'
        if self.debug_enabled:
        ...
        # Put the cause IN the message. status_code and response_text were only ever
        # attributes, so every layer that reads str(exc) -- which is all of them --
        # reported "Failed to delete object" and nothing else. SAP's own sentence
        # ("...is still used by...", "...contains locked objects") lives in the body.
        detail = ''
        ...
        if last_response is None:
            # Genuinely no response: no attempt was made at all. Rare -- and NOT the
            # case that used to land here. Before the `is not None` fix above, every
            # failed delete fell into this shape and was blamed on session state; the
            # attempts had in fact reached SAP and been refused. Kept for the real case,
            # worded so it no longer claims to know why.
            raise SAPADTError(
                "Failed to delete object: no attempt reached SAP (no HTTP response, "
                "and no approach was tried). Cause unknown - check the debug log for "
                "which approaches were built.",
                status_code=None, response_text=None
            )
        raise SAPADTError(
            f"Failed to delete object (HTTP {status})" + (f": {detail}" if detail else ""),
            status_code=status if isinstance(status, int) else None,
            response_text=text[:500] if text else None
        )
    ...
    def create_dataelement(self, name, domain_name, description, package_name,
        ...
        # Build XML body.
        #
        # THIS CREATE PRODUCES A SHELL, NOT A FINISHED DATA ELEMENT. Measured on
        # NS4/NR4, 2026-08-11: the endpoint returns 201 for several different body
        # shapes and stores NONE of the content. Reading the object back afterwards
        # shows <dtel:typeName/> and empty labels, and activation then fails with
        # "No domain or data type was defined".
        #
        # It is the same two-step SAP already imposes on message classes (see
        # adt_message_class: create drops the messages, a separate write puts them
        # in). A data element needs the same treatment -- create, then lock/PUT the
        # real content in the READ representation, then activate. That write step
        # is not implemented here yet, so callers of this method get an object that
        # exists and cannot be activated.
        #
        # Do not "fix" this by reshaping the body. Three variants were tried
        # against the live endpoint (flat typeKind/typeName, flat plus a dataType
        # leaf, and the wrapper form) -- all three returned 201 and all three
        # stored nothing. The read representation is NOT the create representation:
        # posting the read shape (blue:wbobj / adt/dictionary/dataelements) is
        # rejected outright with "System expected the element
        # '{http://www.sap.com/wbobj/dictionary/dtel}wbobj'".
...
<dtel:wbobj adtcore:name="{name.upper()}"
  ...
  <dtel:deactivateBIDIFiltering>false</dtel:deactivateBIDIFiltering>
    ...
    # A domain's OUTPUT length is not its input length: it is how many characters
    # the value occupies on screen, including the sign, the decimal point and the
    # thousands separators. Writing the input length instead produces an
    # activation warning ("Output length (15) is less than the calculated output
    # length (19)") and silently truncates values in ALV and dynpro -- worst on
    # QUAN/CURR, which is exactly where a wrong figure matters.
    #
    # Table from the NTT ARC-1 DDIC playbook, confirmed against a live domain
    # (NR4: INT4 length 10 -> output length 11, while this method was writing 10).
    _DOMA_OUTPUT_LEN = {'INT1': 4, 'INT2': 6, 'INT4': 11, 'INT8': 20}
    _DOMA_OUTPUT_PLUS4 = {'DEC', 'QUAN', 'CURR'}

    @classmethod
    def default_output_length(cls, datatype, length):
        """Screen width for a domain type. Unknown types keep the input length."""
        dt = (datatype or '').upper()
        if dt in cls._DOMA_OUTPUT_LEN:
            return cls._DOMA_OUTPUT_LEN[dt]
        if dt in cls._DOMA_OUTPUT_PLUS4:
            return int(length) + 4
        return int(length)
    ...
    def create_domain(self, name, datatype, length, description, package_name,
        ...
        if fixed_values:
            ...
            for idx, fv in enumerate(fixed_values, start=1):
                # Escaped like every other text this file writes into ADT XML
                # (self._xml_escape, used since :4144). Without it a value text
                # carrying & or < -- ordinary in a description -- produces XML SAP
                # rejects, and the failure names the payload, not the character.
                value = self._xml_escape(fv.get('value', ''))
                text = self._xml_escape(fv.get('text', fv.get('value', '')))
      ...
      <doma:text>{text}</doma:text>
        ...
        # Caller's value wins; otherwise derive it. Never reuse length_str here.
        out_len = (int(output_length) if output_length
                   else self.default_output_length(datatype, length))
        out_len_str = str(out_len).zfill(6)
...
<doma:domain adtcore:name="{name.upper()}"
  ...
  <doma:content>
    ...
    <doma:outputInformation>
      <doma:length>{out_len_str}</doma:length>
...
<blue:blueSource xmlns:blue="http://www.sap.com/wbobj/blue"
  ...
  <blue:source>{ddl_source}</blue:source>
        ...
        def make_request():
            # Rebuild headers inside the closure so CSRF-refresh retries pick up
            # the fresh token — same reason as create_cds_view above.
            headers = self._get_headers(
    ...
    def _validate_cds_source(self, cds_source):
        ...
        # A DDL source is not always "DEFINE VIEW ... SELECT FROM". The literal
        # substring check rejected every modern form whose words are not adjacent
        # -- `define root view entity` most importantly, which is what every RAP
        # root is -- while `define view entity` passed only by accident, its
        # uppercase happening to contain "DEFINE VIEW". And SELECT FROM is absent
        # by design from projections (`as projection on`), abstract and custom
        # entities, and hierarchies.
        cds_upper = cds_source.upper()
        statement = re.search(
            r'\bDEFINE\s+(?:ROOT\s+|TRANSIENT\s+|ABSTRACT\s+|CUSTOM\s+)*'
            r'(?:VIEW|ENTITY|HIERARCHY)\b', cds_upper)
        if not statement:
            raise SAPValidationError(
                "CDS source must contain a DEFINE ... VIEW/ENTITY/HIERARCHY statement "
                "(e.g. 'define view entity', 'define root view entity', "
                "'define abstract entity')",
                field='cds_source'
            )
        # A data source is required only for the forms that read one. Abstract and
        # custom entities declare a shape and select from nothing.
        declares_only = re.search(r'\bDEFINE\s+(?:ABSTRACT|CUSTOM)\s+ENTITY\b', cds_upper)
        if not declares_only and not re.search(
                r'\bSELECT\s+FROM\b|\bPROJECTION\s+ON\b|\bHIERARCHY\s*\(', cds_upper):
            raise SAPValidationError(
                "CDS source names no data source: expected SELECT FROM, "
                "PROJECTION ON, or a HIERARCHY(...) clause",
                field='cds_source'
            )
  ...
  <ddl:sourceMainArtifact>
    ...
    <ddl:source>{cds_source}</ddl:source>
        ...
        def make_request():
            # Rebuild headers inside the closure so CSRF-refresh retries pick up
            # the fresh token (see fetch_csrf_token force_refresh path).
            # Built once outside, this closure re-sent the STALE token on every
            # retry: _retry_request refreshes self.csrf_token but cannot reach a
            # headers dict it never sees, so the call burned all three attempts
            # on "CSRF token validation failed" and reported "Unknown error".
            headers = self._get_headers(
    ...
    # Source-bearing object types: the definition IS its text, so they are created
    # as an EMPTY shell and populated by a normal source push. Every entry was read
    # off a live system before being written down (NR4/NS4, 2026-08-11) -- the
    # media types in particular are not guessable, and a wrong Accept answers 406
    # in a way that reads exactly like "object missing".
    #
    # Tables, structures and behaviour definitions share one root element
    # (blue:blueSource) and differ only in path, adtcore type and media type.
    # Service definitions have their own root, hence the per-kind `root`/`ns`.
    _BLUE_NS = ('xmlns:blue="http://www.sap.com/wbobj/blue" '
                'xmlns:abapsource="http://www.sap.com/adt/abapsource" '
                'xmlns:adtcore="http://www.sap.com/adt/core"')
    _SRVD_NS = ('xmlns:srvd="http://www.sap.com/adt/ddic/srvdsources" '
                'xmlns:abapsource="http://www.sap.com/adt/abapsource" '
                'xmlns:adtcore="http://www.sap.com/adt/core"')
    _DDL_NS = ('xmlns:ddl="http://www.sap.com/adt/ddic/ddlsources" '
               'xmlns:abapsource="http://www.sap.com/adt/abapsource" '
               'xmlns:adtcore="http://www.sap.com/adt/core"')

    SOURCE_SHELL_KINDS = {
        'table': {
            'path': 'ddic/tables', 'adt_type': 'TABL/DT',
            'ct': 'application/vnd.sap.adt.tables.v2+xml',
            'root': 'blue:blueSource', 'ns': _BLUE_NS, 'extra': ''},
        'structure': {
            'path': 'ddic/structures', 'adt_type': 'TABL/DS',
            'ct': 'application/vnd.sap.adt.structures.v2+xml',
            'root': 'blue:blueSource', 'ns': _BLUE_NS, 'extra': ''},
        'behaviordefinition': {
            'path': 'bo/behaviordefinitions', 'adt_type': 'BDEF/BDO',
            'ct': 'application/vnd.sap.adt.blues.v1+xml',
            'root': 'blue:blueSource', 'ns': _BLUE_NS, 'extra': ''},
        'servicedefinition': {
            'path': 'ddic/srvd/sources', 'adt_type': 'SRVD/SRV',
            'ct': 'application/vnd.sap.adt.ddic.srvd.v1+xml',
            'root': 'srvd:srvdSource', 'ns': _SRVD_NS,
            # srvdSourceType="S" is what makes it a Service Definition rather than
            # another srvd flavour; it is on every live object read.
            'extra': 'srvd:srvdSourceType="S"'},
        'cds': {
            'path': 'ddic/ddl/sources', 'adt_type': 'DDLS/DF',
            'ct': 'application/vnd.sap.adt.ddlSource+xml',
            'root': 'ddl:ddlSource', 'ns': _DDL_NS, 'extra': '',
            # ddlSource reports a BARE sourceUri; blueSource reports "./<name>/...".
            # Read off a live object rather than assumed -- the shapes really do
            # differ between these endpoints.
            'src_uri': 'source/main'},
    }

    def create_source_object_shell(self, name, kind, description, package_name,
                                   transport=None):
        """Create an EMPTY source-bearing DDIC object, ready for a source push.

        This is the create half of the source-based path: POST the shell, then
        write the real DDL with the normal push. It exists alongside
        create_table()/create_structure() rather than replacing them, because the
        two answer different questions:

          create_table(fields=[...])  build a NEW table from a specification
          this                        reproduce an EXISTING table from its own DDL

        The distinction is not stylistic. Reducing a real customer table to a
        field list drops keys, foreign keys, technical settings and .INCLUDEs --
        everything that makes it that table rather than a table with the same
        columns. Cross-system transfer needs the second question answered, and
        before this the engine could only answer the first (measured: adt_create
        on a table returned "Unsupported object type: TABL/DT").

        Args:
            name: object name
            kind: 'table' or 'structure'
            description: short text (SAP requires one at create)
            package_name: target package
            transport: transport request; required for any transported package

        Returns:
            dict(success, object_url, message). An existing object is SUCCESS --
            create is idempotent here, because the source push that follows is
            what actually carries the content.
        """
        if kind not in self.SOURCE_SHELL_KINDS:
            raise SAPADTError("create_source_object_shell: unsupported kind '%s' "
                              "(expected one of %s)"
                              % (kind, ', '.join(sorted(self.SOURCE_SHELL_KINDS))))
        spec = self.SOURCE_SHELL_KINDS[kind]
        url_path, adt_type, content_type = spec['path'], spec['adt_type'], spec['ct']

        self._validate_object_name(name, kind)
        self._validate_package_name(package_name)
        self._validate_transport(transport)
        ...
        root, extra = spec['root'], (' ' + spec['extra'] if spec['extra'] else '')
        src_uri = spec.get('src_uri') or f'./{name.lower()}/source/main'
        shell_xml = f'''<?xml version="1.0" encoding="UTF-8"?>
<{root} {spec['ns']}{extra}
    abapsource:sourceUri="{src_uri}"
    adtcore:name="{esc(name.upper())}"
    adtcore:type="{adt_type}"
  ...
  <adtcore:packageRef adtcore:uri="/sap/bc/adt/packages/{package_name.lower()}"
        ...
        object_url = f'/sap/bc/adt/{url_path}/{name.lower()}'

        def exists():
            """Is the object there? Accept MUST be the endpoint's own media type.

            With the default Accept this GET answers 406
            ExceptionResourceNotAcceptable, which reads exactly like "missing" --
            and an existence check that mistakes 406 for absence is worse than no
            check at all.
            """
            try:
                r = self.session.get(f"{self.url}{object_url}",
        ...
        # ASK BEFORE CREATING. Not an optimisation -- the only reliable order.
        # _retry_request treats a 4xx as retryable and, after three attempts,
        # raises SAPConnectionError("Unknown error"): the create response never
        # comes back, so no amount of status inspection afterwards can recognise
        # "already exists". Measured on NS4 2026-08-11, where a second create cost
        # three round trips and then reported a connection problem for an object
        # sitting right there. Probing first also makes a re-run after a partial
        # transfer cheap instead of three-times-slow on everything that landed.
        if exists():
            return {'success': True, 'object_url': object_url,
                    'message': f'{kind} {name.upper()} already exists'}

        # The existence probe above is a GET on the same stateful session, and it
        # can leave the CSRF token stale. Refresh AFTER probing, not before.
        self.fetch_csrf_token(force_refresh=True)

        def make_request():
            # Headers are built per attempt, deliberately. _retry_request fetches
            # a fresh CSRF token and retries on 403 "CSRF token validation
            # failed" -- but a header dict captured once, outside the closure,
            # keeps sending the stale token and every retry fails the same way.
            # The retry then reports "Unknown error after 3 retries", which says
            # nothing about the actual 403.
            return self.session.post(
                f"{self.url}/sap/bc/adt/{url_path}",
        ...
        try:
            response = self._retry_request(
                make_request, operation=f'create {kind} shell {name}')
        except Exception as exc:
            # The create may still have landed before whatever went wrong. SAP is
            # the source of truth, not the response we did or did not get.
            if exists():
                return {'success': True, 'object_url': object_url,
                        'message': f'{kind} {name.upper()} created '
                                   f'(verified by existence after: {exc})'}
            raise

        if response.status_code in (200, 201) or exists():
            return {'success': True, 'object_url': object_url,
                    'message': f'{kind} shell {name.upper()} created'}

        raise SAPADTError(
            f"Failed to create {kind} shell {name}",
            status_code=response.status_code,
            response_text=response.text,
            endpoint=f'/sap/bc/adt/{url_path}')

    SRVB_PATH = 'businessservices/bindings'
    SRVB_CT = 'application/vnd.sap.adt.businessservices.servicebinding.v2+xml'

    def create_service_binding(self, name, description, package_name,
                               service_definition, binding_version='V4',
                               binding_category='0', contract=None,
                               binding_type='ODATA', transport=None):
        """Create a service binding (SRVB) bound to an existing service definition.

        A binding is not source-bearing: the object IS this XML. The caller passes
        the version/category/(contract) triple READ FROM THE SOURCE OBJECT rather
        than a binding-kind label, and that is deliberate -- measured on NR4/NS4
        2026-08-11:

          - the NAME does not tell you the kind. ZFI011_SB_ORD_DLV_DTL_O4 is a V2
            binding; ZOO005_ODS_MATERIAL_V2 is a V4 one.
          - `contract` is a property of the SYSTEM, not of the kind: every readable
            binding on NS4 carries contract="C1" including a V2 one, while the
            equivalent object on NR4 carries none at all.

        A label -> triple mapping therefore has to guess where the source already
        holds the answer, so this API takes the answer.

        `service_definition` must be the name ON THE TARGET. A binding that keeps
        the source SRVD name activates cleanly and serves the wrong definition.

        Note that creating a binding is not delivering it -- see
        publish_service_binding().
        """
        self._validate_object_name(name, 'service binding')
        self._validate_package_name(package_name)
        self._validate_transport(transport)
        if not service_definition:
            raise SAPADTError("create_service_binding: service_definition is "
                              "required; a binding with no definition is not a "
                              "thing SAP can create")

        object_url = f'/sap/bc/adt/{self.SRVB_PATH}/{name.lower()}'

        def exists():
            try:
                r = self.session.get(f"{self.url}{object_url}",
                                     headers=self._get_headers(self.SRVB_CT,
                                                               self.SRVB_CT),
                                     timeout=self.timeout_short)
                return r.status_code == 200
            except Exception:
        ...
        if exists():
            return {'success': True, 'object_url': object_url,
                    'message': f'service binding {name.upper()} already exists'}

        def esc(v):
            return (str(v).replace('&', '&amp;').replace('<', '&lt;')
                    .replace('>', '&gt;').replace('"', '&quot;'))

        contract_attr = f' srvb:contract="{esc(contract)}"' if contract else ''
        # implementation is left empty on purpose: SAP fills it with the binding's
        # own name at create time (measured on four live bindings, all of which
        # carry their own name there).
        binding_xml = f'''<?xml version="1.0" encoding="UTF-8"?>
<srvb:serviceBinding xmlns:srvb="http://www.sap.com/adt/ddic/ServiceBindings"
                     xmlns:adtcore="http://www.sap.com/adt/core"{contract_attr}
    adtcore:name="{esc(name.upper())}"
  ...
  <adtcore:packageRef adtcore:uri="/sap/bc/adt/packages/{package_name.lower()}"
                      adtcore:type="DEVC/K"
                      adtcore:name="{esc(package_name.upper())}"/>
  <srvb:services srvb:name="{esc(name.upper())}">
    <srvb:content srvb:version="0001">
      <srvb:serviceDefinition adtcore:type="SRVD/SRV"
                              adtcore:name="{esc(service_definition.upper())}"/>
    </srvb:content>
  </srvb:services>
  <srvb:binding srvb:type="{esc(binding_type)}" srvb:version="{esc(binding_version)}"
                srvb:category="{esc(binding_category)}">
    <srvb:implementation adtcore:name=""/>
        ...
        def make_request():
            # Headers per attempt: a dict captured outside the closure keeps
            # sending a stale CSRF token through every retry.
            return self.session.post(
                f"{self.url}/sap/bc/adt/{self.SRVB_PATH}",
                headers=self._get_headers(self.SRVB_CT, self.SRVB_CT),
                data=binding_xml.encode('utf-8'),
                params=params,
                timeout=self.timeout_default)

        try:
            response = self._retry_request(
                make_request, operation=f'create service binding {name}')
        except Exception as exc:
            if exists():
                return {'success': True, 'object_url': object_url,
                        'message': f'service binding {name.upper()} created '
                                   f'(verified by existence after: {exc})'}
            raise
        ...
        if response.status_code in (200, 201) or exists():
            return {'success': True, 'object_url': object_url,
                    'message': f'service binding {name.upper()} created'}
        raise SAPADTError(f"Failed to create service binding {name}",
                          status_code=response.status_code,
                          response_text=response.text,
                          endpoint=f'/sap/bc/adt/{self.SRVB_PATH}')

    def unpublish_service_binding(self, name, odata_version='V4',
                                  service_version='0001'):
        """Withdraw a binding's local service endpoint. The inverse of publish.

        Added 2026-08-20 because the engine could create a published service it
        could not remove: SAP refuses to delete a published binding (`412 Service
        endpoint of Service Binding X is published`), and refuses to delete the
        service definition while the binding exists (`404 Prerequisites ... are not
        fulfilled`). A whole RAP stack was therefore undeletable, and clearing it
        needed another client (run 3 / COMPARISON3, gap 2).

        Same shape as publish -- ADT's own inverse job -- so if publish works here,
        this does too. `published` in the result is what the caller should check;
        `adt_service_binding_status` reads `srvb:published` off the object for an
        independent answer.
        """
        base = 'odatav2' if str(odata_version).upper().endswith('2') else 'odatav4'
        svc = name.upper()
        url = (f"{self.url}/sap/bc/adt/businessservices/{base}/unpublishjobs"
               f"?servicename={svc}&serviceversion={service_version}")
        body = ('<adtcore:objectReferences '
                'xmlns:adtcore="http://www.sap.com/adt/core">'
                f'<adtcore:objectReference adtcore:name="{svc}"/>'
                '</adtcore:objectReferences>')

        self.fetch_csrf_token(force_refresh=True)
        headers = self._get_headers('application/*', 'application/xml')
        ...
        severity = pick('SEVERITY') or ('OK' if response.status_code == 200 else 'ERROR')
        ok = severity.upper() != 'ERROR'
        return {'severity': severity,
                'short_text': pick('SHORT_TEXT'),
                'long_text': pick('LONG_TEXT'),
                # `published` is the state AFTERWARDS, so it reads the same way as
                # publish's: True means there is a live endpoint. Unpublishing
                # successfully therefore reports published=False.
                'published': not ok,
                'unpublished': ok,
                'status_code': response.status_code}

    def publish_service_binding(self, name, odata_version='V4',
                                service_version='0001'):
        """Publish a binding's local service endpoint. Activation is NOT delivery.

        An activated but unpublished binding sits in the object list looking
        healthy while the OData service does not exist -- found live on NS4, where
        a test-harness binding had been sitting unpublished. `srvb:published` on
        the binding XML is the readable proof, so the caller can verify rather
        than trust this call's answer.

        Returns dict(severity, short_text, long_text, published). A non-ERROR
        severity means published. "Service Definition ... not yet active" is the
        known transient: publish belongs at the END of a run, after the whole RAP
        stack has activated.
        """
        base = 'odatav2' if str(odata_version).upper().endswith('2') else 'odatav4'
        svc = name.upper()
        url = (f"{self.url}/sap/bc/adt/businessservices/{base}/publishjobs"
               f"?servicename={svc}&serviceversion={service_version}")
        body = ('<adtcore:objectReferences '
                'xmlns:adtcore="http://www.sap.com/adt/core">'
                f'<adtcore:objectReference adtcore:name="{svc}"/>'
        ...
        return {'severity': severity,
                'short_text': pick('SHORT_TEXT'),
                'long_text': pick('LONG_TEXT'),
                'published': severity.upper() != 'ERROR',
                'status_code': response.status_code}
...
<atypgr:abapTypeGroup xmlns:atypgr="http://www.sap.com/adt/ddic/typegroups"
  ...
  <adtcore:packageRef adtcore:uri="/sap/bc/adt/packages/{package_name.lower()}"
        ...
        # Through _request_with_csrf_retry: a bare POST dies on a stale cached
        # token with a flat 403 and no recovery. The helper refreshes the token
        # AND writes it back into these headers.
        response = self._request_with_csrf_retry(
            'post',
        ...
        # Through set_object_source, not a bare PUT: writing an object's source
        # needs a lock handle and the CTS registration that goes with it. The bare
        # PUT answered 400 with nothing to act on.
        self.set_object_source(f"{object_url}/source/main", source_code,
                               lock_handle=None, transport=transport)
    ...
    def set_function_module_source(self, name, function_group, source_code,
        ...
        try:
            ...
            if lock.status_code != 200:
                # Put SAP's own reason in the MESSAGE, not only in response_text.
                # Callers print str(exc), so "FM lock failed" was all anyone saw —
                # and that label covers two unrelated causes: a stale CSRF token
                # (403 "CSRF token validation failed") and a genuine edit lock
                # (403 "user X is already editing"). Hiding the difference sent a
                # whole afternoon hunting locks that did not exist.
                raise SAPADTError(
                    f"FM lock failed for {name}: {_adt_reason(lock)}",
                    status_code=lock.status_code,
                    response_text=lock.text[:4000], endpoint=fm_url)
            lock_handle = next(
    ...
    def run_atc_check(self, object_url, variant='DEFAULT', max_verdicts=100):
        ...
        if response1.status_code not in [200, 201]:
            raise SAPADTError(
                ...
                response_text=response1.text[:4000]
            )
      ...
      <adtcore:objectReferences>
        ...
        if response2.status_code not in [200, 201]:
            raise SAPADTError(
                ...
                response_text=response2.text[:4000]
            )
        ...
        if response3.status_code != 200:
            ...
            if response3.status_code != 200:
                raise SAPADTError(
                    ...
                    response_text=response3.text[:4000]
                )
...
<ddlx:ddlxSource xmlns:ddlx="http://www.sap.com/adt/ddic/ddlxsources"
  ...
  <adtcore:packageRef adtcore:uri="/sap/bc/adt/packages/{package_name.lower()}"
        ...
        headers = self._get_headers(
            # Media type taken from /sap/bc/adt/discovery for this collection;
            # 'ddlxSource+xml' answers 415.
        ...
        # Through _request_with_csrf_retry, not session.post: a bare POST here
        # dies on a stale cached token with a flat 403 and no recovery. That
        # helper refreshes the token AND writes it back into these headers --
        # the half _retry_request's closure form cannot do.
        response = self._request_with_csrf_retry(
            'post',
        ...
        # The shell exists but the OBJECT is the source. Swallowing an upload
        # failure here and returning success anyway ships an empty metadata extension that
        # reports as created -- the exact class of lie the engine repair exists to
        # remove. Fatal, and named.
        ...
        try:
            ...
            source_error = str(e)
            if self.debug_enabled:
                self._debug(f"[DEBUG] DDLX source upload failed: {e}")

        return {
            'success': source_error is None,
            'source_uploaded': source_error is None,
...
<acm:dclSource xmlns:acm="http://www.sap.com/adt/acm/dclsources"
  ...
  <adtcore:packageRef adtcore:uri="/sap/bc/adt/packages/{package_name.lower()}"
        ...
        headers = self._get_headers(
            # Discovery calls it dclSource+xml -- no 'acm' segment, though the
            # collection lives under /acm/. The 'acm.' form answers 415.
        ...
        # The shell exists but the OBJECT is the source. Swallowing an upload
        # failure here and returning success anyway ships an empty access control that
        # reports as created -- the exact class of lie the engine repair exists to
        # remove. Fatal, and named.
        # Upload the source (pass transport so SAP registers write under correct CTS entry)
        source_error = None
        try:
            ...
            source_error = str(e)
            if self.debug_enabled:
                self._debug(f"[DEBUG] DCL source upload failed: {e}")
    ...
    def create_behavior_definition(self, name, root_entity, package_name, source,
                                   description='', transport='', activate=True):
        """Create an ABAP Behavior Definition (BDEF) from its source.

        The shell comes from create_source_object_shell, which already holds the
        path, media type and root element for this type in SOURCE_SHELL_KINDS.
        The bespoke document this used to build was wrong in four separate ways
        (endpoint, CSRF handling, media type, root element), each discovered one
        HTTP status at a time; there is no reason for a fifth hand-rolled XML
        document in this file when a correct one already exists.

        `implementation_type` is gone with it. It only fed the generated starter
        body, and a caller creating a BDEF has the body -- so `source` is now
        required rather than optional.

        Args:
            name: Behavior Definition name (e.g. 'ZI_MY_BDEF')
            root_entity: the root entity the behaviour is defined for. Checked
                         against the source rather than sent to SAP: the binding
                         lives in `define behavior for <entity>`, and a mismatch
                         between what the caller believes and what the source says
                         surfaces at activation as something far less obvious.
            package_name: Package name
            source: BDEF source code (required)
            description: Description text
            transport: Transport request number
            activate: Activate after the source lands

        Returns:
            dict with success / source_uploaded / activated / error
        """
        ...
        if not source or not str(source).strip():
            raise SAPValidationError(
                "BDEF source is required (implementation_type generation was removed)",
                field='source'
            )

        if root_entity:
            declared = re.search(r'DEFINE\s+BEHAVIOR\s+FOR\s+([A-Z0-9_/]+)',
                                 str(source).upper())
            if declared and declared.group(1) != root_entity.upper():
                raise SAPValidationError(
                    "root_entity is %s but the source defines behavior for %s"
                    % (root_entity.upper(), declared.group(1)),
                    field='root_entity'
                )

        shell = self.create_source_object_shell(
            name=name, kind='behaviordefinition',
            description=description or name,
            package_name=package_name, transport=transport or None)
        if not (shell or {}).get('success'):
            raise SAPADTError(
                "Failed to create behavior definition shell: %s"
                % (shell or {}).get('message', 'no detail'))

        object_url = shell.get('object_url') or \
            f'/sap/bc/adt/bo/behaviordefinitions/{name.lower()}'

        # Upload source if provided. A swallowed failure here returns an empty
        # object reported as created; the source IS the object, so this is fatal.
        source_error = None
        if source:
            try:
                self.set_object_source(f"{object_url}/source/main", source, lock_handle=None, transport=transport)
                ...
                source_error = str(e)
                if self.debug_enabled:
                    self._debug(f"[DEBUG] BDEF source upload failed: {e}")

        # Activation is reported, not swallowed, but it is NOT fatal: a behaviour
        # object over dependencies that are still inactive legitimately fails to
        # activate, and the fix is to activate them, not to recreate this.
        activated, activation_error = None, None
        if activate and source_error is None:
            try:
                ...
                if self.debug_enabled:
                    self._debug(f"[DEBUG] BDEF activation failed: {e}")
        ...
        return {
            ...
            'activated': activated,
            'error': source_error or '',
            'activation_error': activation_error or '',
            'object_url': object_url,
            'name': name.upper(),
...
<bimpl:behaviorImplementation xmlns:bimpl="http://www.sap.com/adt/behaviorImplementations"
  ...
  <adtcore:packageRef adtcore:uri="/sap/bc/adt/packages/{package_name.lower()}"
                      adtcore:type="DEVC/K"
                      adtcore:name="{package_name.upper()}"/>
  <bimpl:behaviorDefinitionRef adtcore:uri="/sap/bc/adt/bo/behaviordefinitions/{behavior_definition.lower()}"
        ...
        if source:
            try:
                ...
                source_error = str(e)
                if self.debug_enabled:
                    self._debug(f"[DEBUG] BIMP source upload failed: {e}")
        ...
        if activate and source_error is None:
            try:
                ...
                activation_error = str(e)
                if self.debug_enabled:
                    self._debug(f"[DEBUG] BIMP activation failed: {e}")
